...
## This small viz without the coverage plot takes about a minute to
## compile. Can we speed that up?
system.time({
  gg2animint(viz, "boxes-no-coverage")
})
##    user  system elapsed 
##  56.736   1.664  58.943
...
## TODO: this coverage plot takes forever to compile. Can we speed
## that up?
bigviz <- viz
bigviz$coverage <- ggplot()+
  geom_line(aes(pos.norm, cov.norm, group=TYPE, color=TYPE,
                showSelected=insert),
            data=scaffolds$big.norm)+
  scale_color_manual("sequence", values=type.colors)+
  ggtitle("Sequences around selected insert")
system.time({
  gg2animint(bigviz, "boxes-coverage")
})
##    user  system elapsed 
## 925.328  44.792 977.725 
## > 925.328/60
## [1] 15.42213
