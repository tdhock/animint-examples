...
unaligned <- list(
  ...
  selector.types=list(set.name="single"),
...
## signal and probability plots separate, update axes in both.
unaligned.update <- list(
  chroms=ggplot()+
    ggtitle("select annotated set")+
    theme_bw()+
    theme_animint(width=250, height=220)+
  ...
  probability=ggplot()+
    ...
    theme_bw()+
    theme_animint(width=1300, height=300, update_axes="x")+
    ...
    scale_fill_manual(values=ann.colors)+
    geom_hline(yintercept=1/2, color="grey")+
    geom_ribbon(aes(mid.base/1e3, ymin=min.prob, ymax=max.prob,
                    showSelected=sample1,
                    showSelected2=sample2,
                    showSelected3=complexity.i,
                    showSelected4=set.name),
  ...
  signal=ggplot()+
    ggtitle("ChIP-seq signal pair")+
    theme_bw()+
    theme_animint(width=1300, height=300, update_axes=c("x", "y"))+
    xlab("position on chromosome (kb = kilo bases)")+
    scale_y_continuous("<---- one signal ----- another signal ->")+
    scale_fill_manual(values=ann.colors)+
    geom_tallrect(aes(xmin=chromStart/1e3, xmax=chromEnd/1e3,
...
for(selector.name in names(chip.seq$samples)){
  ...
  unaligned.update$samples <- unaligned.update$samples+
    geom_text(aes_string(clickSelects=selector.name, x="x", y="y",
                         showSelected="set.name", label="label"),
              data=text.df, hjust=1)+
    geom_rect(aes_string(xmin="xmin", xmax="xmax", ymin="ymin", ymax="ymax",
...
animint2dir(unaligned.update, "chip-seq-unaligned-update")
...
aligned <- list(
  ...
  error=ggplot()+
    ...
    ggtitle("select model complexity and set")+
    xlab("model complexity -log(lambda)")+
    ylab("incorrect/total labels (percent)")+
    geom_vline(aes(xintercept=complexity,
                   clickSelects=complexity.i),
               data=chip.seq$models, size=15, alpha=1/2)+
    scale_color_manual(values=fp.fn.colors)+
  ...
  probSignal=ggplot()+
    theme_bw()+
    theme_animint(width=1300, height=600)+
...
animint2dir(aligned, "chip-seq-aligned")

## theme_animint(updateAxes=c("x", "y")) Update the x and y axes after
## the selection changes, instead of drawing geom_text labels for the
## min/max values.
prob.df <-
...
updateAxes <- list(
  ...
  probSignal=ggplot()+
    ...
    theme_bw()+
    theme(panel.margin=grid::unit(0, "lines"))+
    theme_animint(width=1300, height=600, update_axes=c("x", "y"))+
    facet_grid(facet ~ ., scales="free_y")+
    xlab("position on chromosome (kb = kilo bases)")+
    ylab("")+
    geom_tallrect(aes(xmin=chromStart/1e3, xmax=chromEnd/1e3,
...
animint2dir(updateAxes, "chip-seq-updateAxes")
